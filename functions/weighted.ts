...
export const onRequestGet = async ({ request, env }: any) => {
  try {
      ...
      const img = it.image_url 
        ...
        : `<div class=\"w-full h-40 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center\">\
             <svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 400 300\" class=\"w-full h-full max-w-40 max-h-32\">\
               <defs>\
                 <linearGradient id=\"bg-weighted\" x1=\"0%\" y1=\"0%\" x2=\"100%\" y2=\"100%\">\
                   <stop offset=\"0%\" style=\"stop-color:#fef3c7;stop-opacity:1\" />\
                   <stop offset=\"100%\" style=\"stop-color:#fbbf24;stop-opacity:1\" />\
                 </linearGradient>\
                 <linearGradient id=\"hat-weighted\" x1=\"0%\" y1=\"0%\" x2=\"100%\" y2=\"100%\">\
                   <stop offset=\"0%\" style=\"stop-color:#dc2626;stop-opacity:1\" />\
                   <stop offset=\"100%\" style=\"stop-color:#991b1b;stop-opacity:1\" />\
                 </linearGradient>\
                 <linearGradient id=\"utensil-weighted\" x1=\"0%\" y1=\"0%\" x2=\"100%\" y2=\"100%\">\
                   <stop offset=\"0%\" style=\"stop-color:#374151;stop-opacity:1\" />\
                   <stop offset=\"100%\" style=\"stop-color:#1f2937;stop-opacity:1\" />\
                 </linearGradient>\
               </defs>\
               <rect width=\"400\" height=\"300\" fill=\"url(#bg-weighted)\" />\
               <path d=\"M200 80 C180 80, 160 100, 160 120 L160 140 C160 150, 170 160, 180 160 L220 160 C230 160, 240 150, 240 140 L240 120 C240 100, 220 80, 200 80 Z\" fill=\"url(#hat-weighted)\" />\
               <ellipse cx=\"200\" cy=\"170\" rx=\"50\" ry=\"8\" fill=\"url(#hat-weighted)\" />\
               <rect x=\"180\" y=\"180\" width=\"8\" height=\"40\" fill=\"url(#utensil-weighted)\" />\
               <rect x=\"212\" y=\"180\" width=\"8\" height=\"40\" fill=\"url(#utensil-weighted)\" />\
               <path d=\"M175 200 Q180 195, 185 200 Q190 205, 195 200 Q200 195, 205 200 Q210 205, 215 200 Q220 195, 225 200\" stroke=\"url(#utensil-weighted)\" stroke-width=\"3\" fill=\"none\" />\
    ...
    const base = url.origin + "/weighted/";
...
<body class="bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-100">
  ${renderHeader({
    ...
    rightHtml: '<form method="GET" action="/weighted/" class="flex items-center gap-2">\
      <input class="w-52 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2" type="text" name="q" value="' + escapeHtml(q || '') + '" placeholder="搜索加权热榜" />\
    ...
    mobileExtraHtml: '<form method="GET" action="/weighted/" class="flex items-center gap-2">\
      <input class="flex-1 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2" type="text" name="q" value="' + escapeHtml(q || '') + '" placeholder="搜索加权热榜" />\
      <button class="inline-flex items-center px-3 py-2 rounded-md border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200" type="submit">搜索</button>\
    </form>'
  })}
  <main class="max-w-4xl mx-auto px-4 py-6">
    <nav aria-label="Breadcrumb" class="text-sm text-slate-500 dark:text-slate-400 mb-4">
      <ol class="flex flex-wrap items-center gap-2">
        ...
        <li>/</li>
        <li class="text-slate-700 dark:text-slate-300">加权</li>
      </ol>
    </nav>
    ...
    <nav class="mt-6 flex gap-2">
      ${page > 1 ? `<a class=\"inline-flex items-center px-3 py-2 rounded-md border bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200\" href=\"/weighted/?page=${page - 1}&limit=${limit}${q ? `&q=${encodeURIComponent(q)}` : ''}\">上一页</a>` : ''}
      ${hasNext ? `<a class=\"inline-flex items-center px-3 py-2 rounded-md border bg-white hover:bg-slate-50 text-slate-700 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200\" href=\"/weighted/?page=${page + 1}&limit=${limit}${q ? `&q=${encodeURIComponent(q)}` : ''}\">下一页</a>` : ''}
    </nav>
  </main>
  ${renderFooter({ widthClass: 'max-w-4xl' })}
    ...
    console.error("SSR /weighted error:", e);
