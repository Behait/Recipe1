...
- `functions/recipes/index.ts`：菜谱列表 SSR（分页与搜索）。
 - `functions/recipes/[id].ts`：菜谱详情 SSR（含 JSON-LD 与 canonical，站内主访问路径）。
 - `functions/recipes/[slug].ts`：兼容旧链接的备用详情 SSR。
...
- 详情页
  - `/recipes/:id`：菜谱详情（站内唯一 URL，长尾流量核心）。
  - `/recipes/:slug`：兼容旧链接的备用入口（不再用于站内链接）。
...
## 12. 变更记录（近期）
- 热门统计：新增 `recipe_hit_stats`（按天聚合命中），用于趋势计算。
- 数据查询：新增本周、近30天、指数衰减加权热榜查询方法（DB helper 与 API）。
- 接口扩展：`GET /api/recipes?sort=popular|week|recent|weighted`；分类 `GET /api/categories`、`GET /api/categories/:slug`、`PUT/DELETE /api/categories/:id`。
- SSR 列表页：支持新排序参数与文案，保留 `sort` 到分页链接。
- 管理页面：`/admin` 分类管理（重命名、删除、给菜谱添加分类）。由 Basic Auth 保护（`ADMIN_USER`、`ADMIN_PASS`）。

## 13. 常见问题（热门/分类/管理）
- 如何调用热门排序？
  - API：
    - 本周：`GET /api/recipes?sort=week&page=1&limit=12`
    - 近期：`GET /api/recipes?sort=recent&page=1&limit=12`
    - 加权：`GET /api/recipes?sort=weighted&page=1&limit=12`
    - 经典热门：`GET /api/recipes?sort=popular`
  - SSR：`/recipes/?sort=week|recent|weighted|popular`。
- `500 Internal Server Error` 常见原因？
  - 未配置 `DB_CONNECTION_STRING` 或 Neon 未执行新表 SQL；请在 Pages 项目变量中设置连接串，并在 Neon 执行 `sql/init_neon.sql`（包含 `recipe_hit_stats`）。
- 管理页面权限？
  - 通过 Basic Auth 保护 `/admin` 与写接口；需在 Pages 变量中设置 `ADMIN_USER` 与 `ADMIN_PASS`。未设置时为了本地开发方便会放行，不建议在线上这样使用。
## 变更记录

2025-10-27
- 修复分类与热门查询的条件拼接：移除嵌套 `sql\`...\`` 片段与 `sql\`TRUE\`` 的用法，统一改为参数化 `CASE WHEN $param IS NULL THEN TRUE ELSE ... END` 条件。
- 目的：避免在 Cloudflare Pages + Neon 环境下，嵌套片段被当作字符串参数注入到 `WHERE`，导致 Postgres 试图将字符串解析为布尔值而报错（如 `invalid input syntax for type boolean`）。
- 影响范围：`listCategories`、`listRecipesByCategoryName`、`listRecipesByCategorySlug`、`listTrendingByWindow`。
- 兼容性：当 `q` 为空或未提供时，查询返回全部结果；当提供 `q` 时，按 `ILIKE` 进行过滤。