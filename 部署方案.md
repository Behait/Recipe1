...
-- 新增（热门统计）：按天聚合命中，用于趋势计算
CREATE TABLE IF NOT EXISTS recipe_hit_stats (
  recipe_id UUID NOT NULL REFERENCES recipes(id) ON DELETE CASCADE,
  hit_date DATE NOT NULL,
  hit_count INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (recipe_id, hit_date)
);
CREATE INDEX IF NOT EXISTS idx_recipe_hit_stats_date ON recipe_hit_stats(hit_date);
CREATE INDEX IF NOT EXISTS idx_recipe_hit_stats_recipe ON recipe_hit_stats(recipe_id);
```
...
- `ADMIN_USER`、`ADMIN_PASS`：用于 Basic Auth 保护 `/admin` 页面与管理写接口（未设置时仅用于本地开发放行，线上必须设置）。
- 生成图片（Vertex AI）相关：
...
- 访问：
  ...
  - 管理中心：`http://127.0.0.1:8789/admin`（如已配置 `ADMIN_USER`/`ADMIN_PASS`，需 Basic Auth）
...
- API：
  ...
  - `GET /api/recipes?sort=week|recent|weighted|popular`（热门排序）
  - `GET /api/recipes/:id`
  - `GET /api/daily/:date`
  - `GET /api/categories`、`GET /api/categories/:slug`、`PUT/DELETE /api/categories/:id`
- SSR 页面：
  - `/recipes/` 列表、`/recipes/[slug]` 详情（含 OG/Twitter + JSON-LD）
  - `/daily/[date]`
  - `/categories/` 索引与 `/categories/[name]` 列表
  - `/admin` 管理中心（需 Basic Auth）
- 抓取辅助：
